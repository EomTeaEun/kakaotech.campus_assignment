...
import { getDateString, getTodoDateKey, getWeekDays } from "../lib/date";
import type { Todo, FilterType } from "../lib/types";

const DAY_NAMES = ["일", "월", "화", "수", "목", "금", "토"];
const FILTER_LABEL: Record<FilterType, string> = { done: "완료", active: "진행중", all: "전체" };

function filterAndSort(todos: Todo[], filter: FilterType): Todo[] {
...
function DailyView({ todos, currentViewDate, currentFilter }: DailyViewProps) {
...
function WeeklyView({ todos, currentViewDate, currentFilter, onSelectDate }: WeeklyViewProps) {
...
interface TodoListPanelProps {
  ...
  currentViewDate: Date;
  currentFilter: FilterType;
  panelViewMode: "daily" | "weekly";
  onToggleMode: (mode: "daily" | "weekly") => void;
  onSelectDate: (dateKey: string) => void;
}

export default function TodoListPanel({ todos, currentViewDate, currentFilter, panelViewMode, onToggleMode, onSelectDate }: TodoListPanelProps) {
  return (
